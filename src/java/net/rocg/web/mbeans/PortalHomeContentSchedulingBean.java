...
@ManagedBean
@SessionScoped
public class PortalHomeContentSchedulingBean {
DBConnection dbConn;
    int loginId, roleId;
   ...
   String contentBasePath,contentBaseURL;
     ...
     */
    public PortalHomeContentSchedulingBean() {
        ...
        if(conn!=null){
            ...
            try{
                ...
                if(addContent){
                    ...
                    if(opId<=0){
                        dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: fetchData() ::Invalid Operator Id , Please select an operator to schedule content for.");
                    ...
                    dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: fetchData() ::Invalid Tab Id , Please select a tab to schedule content for.");
                        ...
                        PreparedStatement pst=conn.prepareStatement("insert into tb_cms_contents_portalmap(content_id,portal_id,tab_id,hp_flag,operator_id,mapping_date,mapped_by,STATUS,show_order) values(?,"+this.getSelectedPortalId()+","+this.getSelectedTabId()+",1,"+this.getSelectedOperatorId()+",now(),"+this.getLoginId()+",1,?);");
   ...
   public void removeContentFromPortal() {  
       ...
       if(this.getSelectedPortalContents().size()>0){ 
           String sql1="delete from tb_cms_contents_portalmap where operator_id="+this.getSelectedOperatorId()+" and hp_flag=1 and tab_id="+this.getSelectedTabId()+"  and content_id in (";
           ...
           sql1 +=")";
           int rep=0;
           dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: removeContentFromPortal() :: sql " + sql1);
           ...
           if(conn!=null){
               try{
                   ...
                   dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: removeContentFromPortal() :: Exception " + e);
           ...
           dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: removeContentFromPortal() :: No content selected to remove from portal.");
   ...
   public void onEdit(RowEditEvent event) {  
      CMSContent actionObj=(CMSContent)event.getObject();
       if(actionObj!=null && actionObj.getContentId()>0){ 
           String sql1="update tb_cms_contents_portalmap set show_order="+actionObj.getShowOrder()+" where portal_id="+this.getSelectedPortalId()+" and hp_flag=1 and tab_id="+this.getSelectedTabId()+" and content_id="+actionObj.getContentId()+";";
           int rep=0;
           dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: onEdit() :: sql " + sql1);
           Connection conn=dbConn.connect();
           if(conn!=null){
               try{
                   Statement st=conn.createStatement();
           ...
           dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: onEdit() :: Invalid Content Id (" + actionObj.getContentId() + ") to update.");
...
public void reloadPortalContentRecords(java.sql.Statement st){
        ...
        try{
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadPortalContentRecords() :: Query : "+sql1);
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadContentRecords() :: Exception "+e);
        }finally{
...
public void reloadContentRecords(java.sql.Statement st){
        ...
        try{
            ...
            StringBuilder sb=new StringBuilder("SELECT a.`content_id`,a.`cat_nr`,a.`content_type_id`,a.`category_id`,a.`copyright_id`,a.`cp_id`,"
                        + "a.`cp_catnr`,a.`content_name`, a.content_folder,CONCAT(a.`content_url`,b.`file_name`,\"_\",b.`parts_count`,\".\",b.`file_extension`) AS content_url FROM tb_cms_contents a, tb_cms_contents_files b WHERE a.`content_id` not in (SELECT content_id FROM tb_cms_contents_portalmap WHERE portal_id="+this.getSelectedPortalId()+" and hp_flag=1 and tab_id="+this.getSelectedTabId()+" and operator_id="+this.getSelectedOperatorId()+") and a.`cat_nr`=b.`cat_nr` AND a.status>=1 AND b.`preview_file`=1 AND b.`file_name` LIKE '%"+previewSize+"'");
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadContentRecords() :: Query : "+sql1);
            rs=st.executeQuery(sql1);
            CMSContent newContent=null;
...
public void reloadCPList(java.sql.Statement st){
    try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadCPList() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            contentCPList.clear();
            while (rs.next()) {
                contentCPList.put(rs.getString("cp_name"),rs.getString("cp_user_id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadCPList() ::  Collection Size "+portalList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadCPList() :: Exception "+e.getMessage());
...
public void reloadTabList(java.sql.Statement st){
    try {
        System.out.println("mapping for :: "+mappingFor);
        String source=(this.getMappingFor().equals("true"))?"menu":"home";
            String sql1="";
            if(source.equals("menu")){
            statusMsg.setMessage("Left Menu Content Mapping Selected!", StatusMessage.MSG_CLASS_INFO);}
            else{
             statusMsg.setMessage("Homepage Content Mapping Selected!", StatusMessage.MSG_CLASS_INFO);   
            }
            
            if(this.getRoleId()==1 || this.getRoleId()==2){
                sql1="select tab_name,id from tb_tab_conf where source='"+source+"' and portal_id="+this.getSelectedPortalId()+";";
            }
            else{
                sql1="select tab_name,id from tb_tab_conf where portal_id="+this.getSelectedPortalId()+" and source='"+source+"';";
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadTABist() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            tabList.clear();
            while (rs.next()) {
                tabList.put(rs.getString("tab_name"),rs.getString("id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadTABist() ::  Collection Size "+portalList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadTABist() :: Exception "+e.getMessage());
...
public void reloadOperatorList(java.sql.Statement st) {
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadOperatorList() ::  Collection Size "+portalList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
...
public void reloadPortalList(java.sql.Statement st) {
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadPortalList() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            portalList.clear();
            while (rs.next()) {
                portalList.put(rs.getString("portal_name"),rs.getString("portal_id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadPortalList() ::  Collection Size "+portalList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadPortalList() :: Exception "+e.getMessage());
...
public void reloadContentTypeList(java.sql.Statement st) {
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadContentTypeList() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            while (rs.next()) {
                contentTypeList.put(rs.getString("content_type_name"), rs.getString("content_type_id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadContentTypeList() ::  Collection Size "+contentTypeList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
           dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadContentTypeList() :: Exception "+e.getMessage());
        }
    }
 ...
 public void reloadSubCategoryList(java.sql.Statement st) {
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadSubCategoryList() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            while (rs.next()) {
                contentSubCategoryList.put(rs.getString("catg_name"), rs.getString("catg_id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadSubCategoryList() ::  Collection Size "+contentSubCategoryList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadSubCategoryList() :: Exception "+e.getMessage());
 ...
 public void reloadContentCategoryList(java.sql.Statement st) {
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadContentCategoryList() :: Query : "+sql1);
            java.sql.ResultSet rs = st.executeQuery(sql1);
            while (rs.next()) {
                contentCategoryList.put(rs.getString("catg_name"), rs.getString("catg_id"));
            }
            dbConn.logUIMsg(RLogger.MSG_TYPE_INFO, RLogger.LOGGING_LEVEL_DEBUG, "PortalHomeContentSchedulingBean.class :: reloadContentCategoryList() ::  Collection Size "+contentCategoryList.size());
            if (rs != null) {
                rs.close();
            }
            rs = null;
        } catch (Exception e) {
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: reloadContentCategoryList() :: Exception "+e.getMessage());
        }
    }
    ...
    public void fetchLoginDetails() {
        ...
        try {
            ...
            dbConn.logUIMsg(RLogger.MSG_TYPE_ERROR, RLogger.LOGGING_LEVEL_ERROR, "PortalHomeContentSchedulingBean.class :: fetchLoginDetails() :: Exception "+e.getMessage());
        }
        loginBeanObj = null;
